...
- **레퍼런스**: Cafe24 EC 관리자 대시보드 **실물** (duarltmd.cafe24.com 2026-07 캡처 기준). 상품명/가격은 모든포장(koreanhc7.cafe24.com) 실제 판매 품목, 주문 데이터는 전부 난수
- **레이아웃** (실물과 동일 구조):
  - 좌측 다크 네이비 사이드바: cafe24 로고, "카페24 PRO 신청하기" 버튼, 홈(활성)/주문/상품/고객/메시지(NEW)/게시판/디자인/프로모션/애널리틱스(NEW)/통계/통합엑셀 + 하단 부가 채널 메뉴
  - 흰 상단바: 쇼핑몰 선택 드롭다운, 사이트캐시 삭제, 통합검색, PRO 가이드, 벨(배지), 시계, AI 어시스턴트 그라데이션 버튼
  - 메인: AD 배너(롤링) → 「오늘의 할 일」 칩 10개(입금전~배송중 5개 + 취소신청~게시물관리 빨간 계열 5개) → 매출 현황 탭 6개 + 일별 막대차트(단위/만원) + 기간별 매출 표(주문/결제/환불, 오늘 행 강조) → 주요 부가 서비스(SMS 잔여건수/전자세금계산서)
  - 우측 컬럼: 광고 배너 카드 + 공지사항 + 업데이트 리스트
  - **진입 팝업 광고** (컨퍼런스 배너, X/"오늘 하루 보지 않기"로 닫기) — 이게 있어야 진짜다
- **자동**:
  - 2.5~5초마다 신규 주문: 오늘 매출/건수·차트 오늘 막대·입금전/배송준비중 칩 증가, SMS 잔여건수 차감(0이 되면 충전 버튼 펄스), 40% 확률로 벨 알림 ("신규 주문: opp접착봉투 × 3")
  - 3초마다 배송 흐름 전이(입금전→배송준비중→배송중→출고), 7초마다 문의/클레임 증감 + 벨 배지
  - 광고 배너 8초 롤링, 차트는 ResizeObserver로 리사이즈 대응
- **데이터**: 전부 하드코딩 난수. 실제 Cafe24 API/관리자 연동 금지
